...
class MixingNetwork(nn.Module):
    ...
    def forward(self, agents_q, state): 
        ...
        w1 = self.hyper_w1(state).view(-1, self.n_agents, self.hidden_dim)  # W1 = [bs, n_agents, hidden_dim]
        b1 = self.hyper_b1(state).view(-1, 1, self.hidden_dim)              # b1 = [bs, 1, hidden_dim]
        
        hidden = F.elu(torch.bmm(agents_q.unsqueeze(1), w1) + b1).squeeze(1)             
        # hidden = [bs, 1, n_agents] * [bs, n_agents, hidden_dim] + [bs, 1, hidden_dim]
        # hidden = [bs, hidden_dim] -> [1, hidden_dim]

        w2 = self.hyper_w2(state).view(-1, self.hidden_dim, 1)              # W2 = [bs, hidden_dim, 1]
        b2 = self.hyper_b2(state)                                          # b2 = [bs, 1] -> [1, 1]

        q_total = torch.bmm(hidden.unsqueeze(1), w2).squeeze(1) + b2
        # q_total = [bs, hidden_dim] * [bs, hidden_dim, 1] + [bs, 1]
        # q_total = [bs, 1] -> [1, 1]
        return q_total.unsqueeze() # [q_total]
...
class ReplayBufferRNN: 
    ...
    def push(self, hidden_seq, state_seq, action_seq, reward_seq, next_state_seq, dones):
        if len(self.buffer) == self.buffer.maxlen:
            self.buffer.popleft()
        data = (hidden_seq, state_seq, action_seq, reward_seq, next_state_seq, dones)
    ...
    def sample(self, batch_size):
        ...
        h_tensor = torch.stack(h_lst).to(self.device)     # (bs, T+1, N, H_dim) T: episode timestep
        s_tensor = torch.stack(s_lst).to(self.device)     # (bs, T, N, obs_dim)
        a_tensor = torch.stack(a_lst).long().to(self.device)     # (bs, T, N, action_dim)
        r_tensor = torch.stack(r_lst).to(self.device)     # (bs, T, N, 1)
        ns_tensor = torch.stack(ns_lst).to(self.device)   # (bs, T, N, obs_dim)
        d_tensor = torch.stack(dn_lst).to(self.device)    # (bs, T, N, 1)
...
class QMIX(nn.Module):
    ...
    def update(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            q_seq, tq_seq = [], []
            for t in range(T):
                h_i = hidden_seq[:, t, i, :] if t < hidden_seq.size(1) - 1 else None
        ...
        metrics = {
            ...
            'avg_entropy': self.epsilon_decay(self.step),
