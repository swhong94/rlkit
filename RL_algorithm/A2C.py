...
class A2C:
  ...
  def __init__(self, state_dim, action_dim, hidden_dim = 64, lr = 1e-3, gamma = 0.99, batch_size =64, device = None):
    ...
    self.buffer = Buffer(batch_size) #Online A2C일 경우에는 매 step마다 업데이트, 그냥 A2C는 N개의 배치마다 update
  ...
  def select_action(self, state):
    ...
    dist = Categorical(action_probs)  # 이산 action space
    action = dist.sample()  
  ...
  def update(self):
    ...
    states, actions, log_probs, rewards, next_states, dones = self.buffer.sample()
    ...
    #Compute critic loss
    actor_loss = -(log_probs * advantage).mean() # detach(): 그래디언트 계산 안해도 돼서 분리리
