...
class IPPO(nn.Module):
    def __init__(self, 
        ...
        for agent in self.agents: 
            ...
            # print(obs_space)
            # assert isinstance(obs_space, gym.spaces.Dict), "Only dictionary observation spaces are supported for now (Discrete will be added later)"
            ...
            input_dim = sum(space.n if isinstance(space, Discrete) else space.shape[0] for space in obs_space.spaces.values()) 
            act_dim = self.env.action_space(agent).n

            self.policies[agent] = PolicyNet(input_dim, hidden_dims, act_dim).to(self.device) 
            self.values[agent] = ValueNet(input_dim, hidden_dims).to(self.device) 
            ...
            self.obs_spaces[agent] = obs_space  # Store for Preprocessing 
    ...
    def preprocess_observation(self, observation, agent): 
        """Convert Dictionary observation to a flat tensor"""
        obs_space = self.obs_spaces[agent]
        one_hots = []
        for key, value in observation.items():
            if isinstance(obs_space.spaces[key], Discrete): 
    ...
    def act(self, state, policy): 
        ...
        state_tensor = self.preprocess_observation(state, agent=self.agents[0])
    ...
    def train_episode(self): 
        ...
        for _ in range(self.max_steps): 
            ...
            for agent in self.agents: 
                ...
                trajectories[agent]['next_states'].append(next_state_tensor)
